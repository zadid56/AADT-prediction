...
for i in ATRs:
    ...
    forecast = model_fit.forecast(steps=len(y_test))[0]
...
np.savetxt("arima.csv", np.column_stack((ATRs,missing_value,hrmse,hmape,armse,amape)), fmt='%.3f', delimiter=",")
np.savetxt("arima_aadt.csv", np.column_stack((AADT_test,AADT_pred)), fmt='%.3f', delimiter=",")
