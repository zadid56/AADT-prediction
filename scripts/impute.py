...
print(np.isnan(np.sum(values)))
imputer = predictive_imputer.PredictiveImputer(f_model='KNN')
temp = imputer.fit(values).transform(values.copy())
#temp = impy.mean(values)
print(values)
print(temp)
print(np.isnan(np.sum(temp))) 
...
# for i in range(13, 13+hours):
	# # load dataset
	# dataset = read_csv('1.csv', usecols=[i])
	# values = dataset.values
	...
	# print(values.shape)
	# temp = np.amax(values, axis=0)
	# scaler = temp.reshape(1,n_col)
	...
	# # design network
	# model = Sequential()
	# model.add(LSTM(n_out, input_shape=(train_X.shape[1], train_X.shape[2])))
	# #model.add(Dropout(0.4))
	# model.add(Dense(n_out))
	# model.compile(loss='mean_squared_error', optimizer='Adam')
	# # fit network
	# history = model.fit(train_X, train_y, epochs=1500, batch_size=365, validation_data=(test_X, test_y), verbose=2, shuffle=False)
	# # plot history
	...
	# yhat = yhat * scaler[0,int(n_col*n_lag/(n_lag+n_lead)):int(n_col)]
	# yhat = yhat.round()
	...
	# y = test_y * scaler[0,int(n_col*n_lag/(n_lag+n_lead)):int(n_col)]
	# y = y.round()
